...
export const Index: Record<string, any> ={`;
  ...
  for (const item of registry.items) {
    ...
    const filesStringArray = await Promise.all(
      item.files.map(async (file) => {
        ...
        const fileRawContent = await fs.readFile(path.join(process.cwd(), filePath), 'utf-8');
        
        // JSON.stringify safely formats the string and puts wrapped quotes around it
        return `{
      path: "${filePath}",
      content: ${JSON.stringify(fileRawContent)},
      type: "${file.type}",
    }`;
      }),
    );
    ...
    if (item.type === 'registry:example') {
      const sourceRawContent = await fs.readFile(sourcePath, 'utf-8');
      extraFields = `
    component: React.lazy(() => import("${componentPath}")),
    source: ${JSON.stringify(sourceRawContent)},`;
    }
  ...
  "${item.name}": {
    ...
    type: "${item.type}",
    files: [${filesStringArray.join(',')}],${extraFields}
  },`;
  }
